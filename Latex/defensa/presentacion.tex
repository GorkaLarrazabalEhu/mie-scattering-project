...
\begin{frame}{MSTM y construcción del sistema}
  \vspace{6pt}
  \textit{Multiple Sphere T-Matrix} (Mackowski \& Mishchenko), en Fortran y en
  \alert{unidades adimensionales} ($x^j = k a^j$). Ensambla el sistema de Foldy--Lax:
  \begin{equation*}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \textbf{\color{oro}Respuesta individual}
      \begin{itemize}\footnotesize
        \item Coef.\ de Mie $\to \bar{\mathbb{T}}_j$ (diagonal)
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{\color{oro}Acoplamiento}
      \begin{itemize}\footnotesize
        \item Traslación $\mathbb{H}^{jl}$ (teorema de adición)
      \end{itemize}
    \end{column}
  \end{columns}
  % \vspace{8pt}
  % \begin{center}
  %   {\footnotesize\color{pizarra2} Orden multipolar (Wiscombe): $n_{max}^{j} \approx x^j + 4\,(x^j)^{1/3} + 5$.}
  % \end{center}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}\footnotesize
        ...
        \item Convierte un proceso \alert{tedioso y propenso a errores} en algo sistemático.
        \item Cada simulación es \alert{reproducible}.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
