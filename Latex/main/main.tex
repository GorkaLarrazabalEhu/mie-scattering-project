...
que relaciona de forma lineal los campos incidente y dispersado y permite 
caracterizar completamente la interacción entre la onda y la partícula. 
Esta matriz surge directamente de aplicar las condiciones de continuidad 
...
componentes correspondientes del campo dispersado (scattered). Los elementos de la matriz de amplitud
...
\subsection{Expansión en armónicos esféricos vectoriales (2)}
...
cualquierda de las funciones de Bessel esféricas $j_n$, $y_n$, $h_n^{(1)}$ o 
$h_n^{(2)}$.
...
anteriores (Bohren pag 92), logrando para el campo eléctrico incidente y magnético
con el rotacional:
...
\hspace{1em}En la frontera de la esfera, imponemos las condiciones
\begin{equation}
  \left( \mathbf{E}_{int} + \mathbf{E}_{sca} - \mathbf{E}_{1} \right) \times \hat{\mathbf{e}}_r
=
\left( \mathbf{H}_{int} + \mathbf{H}_{sca} - \mathbf{H}_{1} \right) \times \hat{\mathbf{e}}_r
= 0.
\end{equation}

Los campos interno ($\mathbf{E}_{int}$, $\mathbf{H}_{int}$) y 
dispersado ($\mathbf{E}_{sca}$, $\mathbf{H}_{sca}$) también pueden expresarse como 
expansiones en armónicos esféricos vectoriales.
...
fácilmente para los coeficientes del campo dentro de la partícula:
...
\cite{Bohren}\cite{Mishchenko}\cite{Bohren,Mishchenko}
