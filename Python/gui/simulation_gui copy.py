...
class Config:
    ...
    def __init__(self):
        ...
        self.num_spheres = ""
        self.positions = []
        self.incident_beta = "0.d0"
        self.incident_alpha = "0.d0"
        self.length_scale = "1.d0"
        self.gaussian_constant = "0.1d0"
        self.near_field = False
        self.near_field_file = ""
        self.data= {} 
...
class MieTheoryApp:
    ...
    def build_gui(self):
        ...
        # Incident Angles
        self.incident_beta_entry = self.create_label_entry("Incident Beta (deg)", self.config.incident_beta)
        self.incident_alpha_entry = self.create_label_entry("Incident Alpha (deg)", self.config.incident_alpha)
        
        # Length Scale Factor
        self.length_scale_entry = self.create_label_entry("Length Scale Factor", self.config.length_scale)
        ...
        self.gaussian_constant_entry = self.create_label_entry("Gaussian Beam Constant", self.config.gaussian_constant)
    ...
    def load_previous_data(self):
        ...
        if os.path.isfile(file):
            try:
                ...
                self.gaussian_constant_entry.insert(0, self.config.data.get("gaussian_beam_constant", "0.1d0"))
    ...
    def write_input_file(self):
        ...
        self.config.folder_location = self.folder_location_entry.get() or DEFAULT_FOLDER_LOCATION
        self.config.output_file = self.output_file_entry.get()
        self.config.num_spheres = self.num_spheres_entry.get()
        self.config.positions = self.position_entry.get("1.0", "end").splitlines()
        self.config.incident_beta = self.incident_beta_entry.get()
        self.config.incident_alpha = self.incident_alpha_entry.get()
        self.config.length_scale = self.length_scale_entry.get()
        self.config.gaussian_constant = self.gaussian_constant_entry.get() if self.beam_type_var.get() == "Gaussian" else False
        self.config.near_field = self.near_field_var.get()
        self.config.near_field_file = self.near_field_file_entry.get() if self.config.near_field else "f"
        
        folder_input_file = os.path.join(self.config.folder_location, "mstm.inp")
        ...
        try:
            with open(folder_input_file, "w") as f:
                ...
                f.write(f"! {self.config.folder_location}\n")
                f.write("output_file\n")
                f.write(f"{self.config.output_file}\n")
                f.write("number_spheres\n")
                f.write(f"{self.config.num_spheres}\n")
                f.write("sphere_data\n")
                for pos in self.config.positions:
                    f.write(f"{pos}\n")
                f.write("end_of_sphere_data\n")
                f.write("incident_beta_deg\n")
                f.write(f"{self.config.incident_beta}\n")  # User-defined value
                f.write("incident_alpha_deg\n")
                f.write(f"{self.config.incident_alpha}\n")  # User-defined value
                f.write("length_scale_factor\n")
                f.write(f"{self.config.length_scale}\n")  # User-defined value
                f.write("solution_epsilon\n")
                f.write("1.d-8\n")

                # Include Gaussian beam parameters if Gaussian is selected
                if self.beam_type_var == "Gaussian":
                    f.write("gaussian_beam_constant\n")
                    f.write(f"{self.config.gaussian_constant}\n")
                ...
                f.write("t\n" if self.config.near_field else "f\n")

                if self.config.near_field:
                    ...
                    f.write(f"{self.config.near_field_file}\n")

                f.write("end_of_options\n")
                
            # self.show_toast_notification(title="Data Loaded", message="Previous data loaded successfully", duration=3000)
    ...
    def run_simulation(self):
        ...
        result = sb.run("mstm.exe", cwd=self.config.folder_location, stdout=sb.PIPE, stderr=sb.PIPE, text=True)
    ...
    def plot_scattering_matrix(self):
        ...
        scat_mat_file = os.path.join(self.config.folder_location, self.config.output_file)
        print(self.config.folder_location, " xd ",self.config.output_file)
        print(f"file: {scat_mat_file}")
    ...
    def plot_near_field(self):
        ...
        self.write_input_file()
        near_field_file = os.path.join(self.config.folder_location, self.config.near_field_file or "nf.dat")
        if "f" != self.config.near_field_file:
            sb.Popen(["py", python_plot_file, near_field_file])
        else:
            self.show_notification("File not found", "File not found")
