...
class MieTheoryApp:
    ...
    def build_gui(self):
        ...
        # Spheres Position
        tk.Label(
            self.root, text="Spheres Position (x, y, z, radius) and Refractive Index (re, im)").pack()
        self.position_entry = tk.Text(self.root, height=4)
        self.position_entry.pack()

        # Incident Angles
        self.incident_beta_entry = self.create_label_entry(
            "Incident Beta (deg)", self.config.data["incident_beta"])
        self.incident_alpha_entry = self.create_label_entry(
            "Incident Alpha (deg)", self.config.data["incident_alpha"])

        # Length Scale Factor
        self.length_scale_entry = self.create_label_entry(
            "Length Scale Factor", self.config.data["length_scale"])

        # Near Field Calculation
        self.near_field_var = tk.BooleanVar()
        tk.Checkbutton(self.root, text="Near Field Calculation",
                       variable=self.near_field_var).pack()

        # Near Field Output File
        self.near_field_file_entry = self.create_label_entry(
            "Near Field Output File Name")

        # Beam Type
        self.beam_type_var = tk.StringVar(value="Plane Wave")
        tk.Label(self.root, text="Beam Type").pack()
        tk.Radiobutton(self.root, text="Plane Wave",
                       variable=self.beam_type_var, value="Plane Wave").pack()
        tk.Radiobutton(self.root, text="Gaussian",
                       variable=self.beam_type_var, value="Gaussian").pack()

        # Gaussian Beam Constant
        self.gaussian_constant_entry = self.create_label_entry(
            "Gaussian Beam Constant", self.config.data["gaussian_beam_constant"])
        ...
        self.create_button("Plot Scattering Matrix",
                           self.plot_scattering_matrix)
        self.create_button("Plot Near Field", self.plot_near_field)

        self.nearfieldplot = tk.StringVar(value="Poynting Vector")
        tk.Radiobutton(self.root, text="Electric Field",
                       variable=self.nearfieldplot, value="Electric Field").pack()
        # tk.Radiobutton(self.root, text="Magnetic Field", variable=self.nearfieldplot, value="Magnetic Field").pack()
        tk.Radiobutton(self.root, text="Poynting Vector",
                       variable=self.nearfieldplot, value="Poynting Vector").pack()
    ...
    def plot_near_field(self):
        ...
        python_plot_file = self.config.plot_location + "\\plot_nearfield.py"
        self.write_input_file()
        near_field_file = os.path.join(
            self.config.data["folder_location"], self.config.data["near_field_file"] or "nf.dat")
        sb.Popen(["py", python_plot_file, near_field_file, self.nearfieldplot.get()])
